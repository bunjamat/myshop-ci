...
class Approvelistsum extends CI_Controller
{
    private $table = "orders_coupon a"; // กำหนดชื่อตารางข้อมูล
    // กำหนดฟิลด์ข้อมูลในตารางที่ต้องการดึงมาใช้งาน
    private $column_select = array("a.order_checkin_date,sum(a.order_qty) as all_qty,sum(a.order_grand_total) as all_total");
   ...
   /*  private $table_join = array(
    ...
    public function index()
    {
        ...
        if($start_date != ''){
            $this->db->group_start();
            $this->db->where('a.order_checkin_date', $start_date);
            $this->db->group_end();
        }
        // $this->db->where('order_approve', 2);

        $this->db->group_by('a.order_checkin_date');
...
$this->db->group_end();
        ...
        foreach ($query->result_array() as $row) {
            ...
            $data[] = array(
                "no" => ($_page * $_length) + $_i,
                "order_checkin_date" => $row['order_checkin_date'],
                "order_qty" => $row['all_qty'],
                "order_grand_total" => $row['all_total'],
            );
